...
public class AnimatorInflaterCompat {
    ...
    private static void parseAnimatorFromTypeArray(ValueAnimator anim, TypedArray arrayAnimator, TypedArray arrayObjectAnimator, float pixelSize, XmlPullParser parser) {
        long duration = (long) TypedArrayUtils.getNamedInt(arrayAnimator, parser, "duration", 1, CheckLongPressHelper.DEFAULT_LONG_PRESS_TIMEOUT);
    ...
    private static int inferValueTypeOfKeyframe(Resources res, Resources.Theme theme, AttributeSet attrs, XmlPullParser parser) {
        ...
        int valueType = 0;
        TypedValue keyframeValue = TypedArrayUtils.peekNamedValue(a, parser, LauncherSettings.Settings.EXTRA_VALUE, 0);
                    ...
                    if (keyframe2.getFraction() < f) {
                        if (i == 0) {
                            ...
                            while (true) {
                                ...
                                if (type2 >= count - 1) {
                                    break;
                                } else if (keyframeArray[type2].getFraction() >= 0.0f) {
                                    break;
                                } else {
                                    endIndex = type2;
                                    j2 = type2 + 1;
    ...
    private static Keyframe loadKeyframe(Context context, Resources res, Resources.Theme theme, AttributeSet attrs, int valueType, XmlPullParser parser) throws XmlPullParserException, IOException {
        ...
        if (hasValue) {
            if (valueType != 3) {
                switch (valueType) {
                    case 0:
                        keyframe2 = Keyframe.ofFloat(fraction, TypedArrayUtils.getNamedFloat(a, parser, LauncherSettings.Settings.EXTRA_VALUE, 0, 0.0f));
                        break;
                    case 1:
                        break;
                }
            }
            keyframe2 = Keyframe.ofInt(fraction, TypedArrayUtils.getNamedInt(a, parser, LauncherSettings.Settings.EXTRA_VALUE, 0, 0));
        } else {
            if (valueType == 0) {
