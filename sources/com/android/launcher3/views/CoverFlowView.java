...
import com.android.launcher3.views.CoverFlowView.CoverFlowAdapter;
...
public class CoverFlowView<T extends CoverFlowAdapter> extends View {
    ...
    private boolean bOnTouch = false;
    ...
    private void initAttributes(Context context, AttributeSet attrs) {
        ...
        this.VISIBLE_VIEWS = a.getInt(5, 2) >> 1;
        this.reflectHeightFraction = a.getFraction(3, 100, 0, 0.0f);
        if (this.reflectHeightFraction > 100.0f) {
            this.reflectHeightFraction = 100.0f;
        }
        this.reflectHeightFraction /= 100.0f;
        this.reflectGap = a.getDimensionPixelSize(2, 0);
        this.mGravity = CoverFlowGravity.values()[a.getInt(0, CoverFlowGravity.CENTER_VERTICAL.ordinal())];
        this.mLayoutMode = CoverFlowLayoutMode.values()[a.getInt(1, CoverFlowLayoutMode.WRAP_CONTENT.ordinal())];
        a.recycle();
    }

    private void init() {
        this.mSysProviderOpt = ((LauncherApplication) this.mContext.getApplicationContext()).getSysProviderOpt();
    ...
    public final void drawChild(Canvas canvas, int position, float offset) {
        ...
        if (wAndh == null) {
            int[] wAndh2 = {child.getWidth(), child.getHeight()};
            this.mImageRecorder.put(actuallyPosition, wAndh2);
        } else {
    ...
    private void makeChildTransfromer(Bitmap child, int position, float offset) {
        ...
        if (position != 0) {
            scale = 1.0f - (Math.abs(offset) * CARD_SCALE);
        } else {
            scale = 1.0f - (Math.abs(offset) * CARD_SCALE);
        }
        if (scale < 0.85f) {
        ...
        float childHeightScale = originalChildHeightScale * scale2;
        ...
        if (offset < -2.0f) {
            this.translateX = ((((float) leftSpace) / ((float) this.VISIBLE_VIEWS)) * (((float) this.VISIBLE_VIEWS) + offset)) + ((float) this.mCoverFlowPadding.left) + ((offset + 2.0f) * 150.0f);
            ...
            this.translateX = (((((float) this.mWidth) - ((((float) rightSpace) / ((float) this.VISIBLE_VIEWS)) * (((float) this.VISIBLE_VIEWS) - offset))) - ((float) childWidth)) - ((float) this.mCoverFlowPadding.right)) + ((offset - 2.0f) * 150.0f);
        } else {
        ...
        float adjustedChildTranslateY2 = adjustedChildTranslateY;
        this.mChildTransfromer.postTranslate(this.translateX, ((float) this.mChildTranslateY) + adjustedChildTranslateY2);
        int i = rightSpace;
        Bitmap bitmap = child;
        int i2 = leftSpace;
        int leftSpace2 = position;
        int i3 = centerChildWidth;
        float f = offset;
        getCustomTransformMatrix(this.mChildTransfromer, this.mDrawChildPaint, bitmap, leftSpace2, f);
        ...
        this.mReflectionTransfromer.postTranslate(this.translateX, (((float) this.mReflectionTranslateY) * scale2) + adjustedChildTranslateY2);
        getCustomTransformMatrix(this.mReflectionTransfromer, this.mDrawChildPaint, bitmap, leftSpace2, f);
    ...
    private void imageOnTop(int position) {
        ...
        this.mTouchRect.left = (float) ((this.mWidth >> 1) - (512 >> 1));
        this.mTouchRect.top = (float) this.mChildTranslateY;
        this.mTouchRect.right = this.mTouchRect.left + ((float) 512);
        this.mTouchRect.bottom = this.mTouchRect.top + ((float) 600);
        if (this.mCoverFlowListener != null) {
    ...
    private void touchBegan(MotionEvent event) {
        ...
        if (this.m_iUITypeVer == 41) {
            ...
            if (this.m_iModeSet == 9) {
                ...
                rt2 = new Rect(LauncherAnimUtils.ALL_APPS_TRANSITION_MS, 37, 475, 194);
                ...
                rt2 = new Rect(LauncherAnimUtils.ALL_APPS_TRANSITION_MS, 37, 475, 524);
                ...
                rt3 = new Rect(421, 7, 604, 207);
                rt4 = new Rect(625, 41, 793, 181);
                rt5 = new Rect(799, 41, 981, 208);
            }
        } else {
            Rect rt12 = new Rect(200, 10, 335, 175);
            rt2 = new Rect(365, 10, 500, 175);
        ...
        if (rt1.contains((int) event.getX(), (int) event.getY())) {
            ...
            this.mLastModeOffset = this.mStartOffset - 1.0f;
    ...
    public void setSelection(int position) {
        ...
        if (position >= 0 && position < max && this.mTopImageIndex != position) {
            ...
            this.mScroller.startScroll(from, 0, disX, 0, Math.min(Math.abs((position + max) - this.mTopImageIndex), Math.abs(position - this.mTopImageIndex)) * 0);
            invalidate();
        }
    }
    ...
    public boolean isOnTouch() {
        return this.bOnTouch;
    }

    public boolean isbScroll() {
        return this.bScroll;
    }

    public void setbScroll(boolean bScroll2) {
        this.bScroll = bScroll2;
    }

    public static abstract class CoverFlowAdapter {
